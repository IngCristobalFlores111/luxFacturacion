...
$query ="SELECT c.idcliente,fp.`folio_factura`,c.nombre,c.RFC,fp.total,fp.tipo,fp.`fecha` FROM `factura_pendiente` fp INNER JOIN clientes_facturacion c ON c.`idcliente` = fp.`idcliente` WHERE fp.total BETWEEN '$amountFrom' AND '$amountTo' AND fp.fecha BETWEEN '$dateFrom' AND '$dateTo' AND c.idcliente='$idCliente' ORDER BY fp.fecha DESC LIMIT 50";
$out1='';
