...
if($doGeneradas)
{
    if($estado_factura=='3')
    {
        $query ="SELECT fg.folio_factura,fg.idcliente,fg.xml_file,c.nombre,c.RFC,fg.`folio_impreso`,fg.`fecha_timbrado`,fg.`fecha_cancelada`,fg.`montoTotal`,fg.`tipo` FROM `factura_generada` fg INNER JOIN clientes_facturacion c ON c.`idcliente`=fg.`idcliente` WHERE fg.montoTotal BETWEEN '$amountFrom' AND '$amountTo' AND fg.fecha_cancelada='0000-00-00' AND c.idcliente='$idCliente' ORDER BY fg.`fecha_timbrado` DESC LIMIT 50";
...
$query ="SELECT c.idcliente,fp.`folio_factura`,c.nombre,c.RFC,fp.total,fp.tipo,fp.`fecha` FROM `factura_pendiente` fp INNER JOIN clientes_facturacion c ON c.`idcliente` = fp.`idcliente` WHERE fp.total BETWEEN '$amountFrom' AND '$amountTo' AND c.idcliente='$idCliente' ORDER BY fp.fecha DESC LIMIT 50";
$out1='';
