...
$query ="SELECT fg.folio_factura,fg.idcliente,fg.xml_file,c.nombre,c.RFC,fg.`folio_impreso`,fg.`fecha_timbrado`,fg.`fecha_cancelada`,fg.`montoTotal`,fg.`tipo` FROM `factura_generada` fg INNER JOIN clientes_facturacion c ON c.`idcliente`=fg.`idcliente` WHERE fg.fecha_timbrado BETWEEN '$dateFrom' AND '$dateTo' ORDER BY fg.`fecha_timbrado` DESC LIMIT 50";
$out2='';
if($doGeneradas)
{
    if($estado_factura=='3')
    {
        $query ="SELECT fg.folio_factura,fg.idcliente,fg.xml_file,c.nombre,c.RFC,fg.`folio_impreso`,fg.`fecha_timbrado`,fg.`fecha_cancelada`,fg.`montoTotal`,fg.`tipo` FROM `factura_generada` fg INNER JOIN clientes_facturacion c ON c.`idcliente`=fg.`idcliente` WHERE fg.fecha_timbrado BETWEEN '$dateFrom' AND '$dateTo' AND fg.fecha_cancelada='0000-00-00' ORDER BY fg.`fecha_timbrado` DESC LIMIT 50";
...
if($printGeneradas)
{
    ...
    foreach($result as $factura)
    {
        ...
        if($fecha_cancelada=='0000-00-00')
        {
            ...
            <button id='$tipo,$folio_Factura,$id_cliente' type='button' class='btn btn-default c_buttonTable2'><i class='fa fa-cog' style='font-size:18px;' aria-hidden='true'></i></button></td>";
            $out2 = $out2."</tr>";
...
$query ="SELECT c.idcliente,fp.`folio_factura`,c.nombre,c.RFC,fp.total,fp.tipo,fp.`fecha` FROM `factura_pendiente` fp INNER JOIN clientes_facturacion c ON c.`idcliente` = fp.`idcliente` WHERE fp.fecha BETWEEN '$dateFrom' AND '$dateTo' ORDER BY fp.fecha DESC LIMIT 50";
$out1='';
