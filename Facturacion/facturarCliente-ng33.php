...
$ambientePruebas = $request['ambientePruebas'];
...
$date = date('Y_m_d');
if(!$ambientePruebas){
    $datos['cfdi']="../../".$clientAndSystemFolder."Facturacion/facturas/timbradas/xml/".$request['idCliente']."_".$date."_".$folio.".xml";  // si se especifica este parametro, se timbrara la factura
}
else{
 $datos['cfdi']="facturas/pruebas/xml/".$request['idCliente']."_".$date."_".$folio.".xml";  // si se especifica este parametro, se timbrara la factura
}
...
$datos['factura']['tipocomprobante'] =strtoupper($facturaParams['tipo']); //ingreso, egreso
//paramas 3.3
...
$query="SELECT regimenes_tipo_factura.idTipoFactura AS tipo_factura,f4_c_regimenfiscal.c_RegimenFiscal AS regimen FROM regimenes_tipo_factura INNER JOIN f4_c_regimenfiscal ON f4_c_regimenfiscal.id = regimenes_tipo_factura.idRegimen
 WHERE regimenes_tipo_factura.idTipoFactura = ?";
$config = $sql->get_bind_results($query,array("i",$facturaParams['tipo_factura']));
$tipo =$config[0]['tipo_factura'];
$regimenFiscalEmisor = $config[0]['regimen'];


$result =$sql->executeQuery("SELECT archivo_cer,archivo_key,csdPass,email FROM usuario_facturacion");
$emailEmisor = $result[0]['email'];
...
if($ambientePruebas){
    ...
    $datos['conf']['cer'] = 'cfdi3.3/certificados/lan7008173r5.cer.pem';
    $datos['conf']['key'] = 'cfdi3.3/certificados/lan7008173r5.key.pem';
    $datos['conf']['pass'] = '12345678a';

}
else{
$datos['PAC']['usuario'] = $emisor['RFC'];;
//$datos['PAC']['pass'] = 'MULTI6d9180f6da47e579158f09226afeea12!';
$datos['PAC']['pass'] ="MULTI54ac3e839c595d1d57a244afdf616e94!";
$datos['PAC']['produccion'] = 'SI'; //   [SI|NO]
$datos['conf']['cer'] ="../../".$clientAndSystemFolder."Facturacion/pruebas/$fileCer";
$datos['conf']['key'] = "../../".$clientAndSystemFolder."Facturacion/pruebas/$fileKey";
...
//    $concepto['descripcion'] = "PRODUCTO PRUEBA > '$i'";
    ...
    if(isset($c['pedimento'])){
        // concepto con info aduanera
        $pedimento =$c['pedimento'];
        $concepto['InformacionAduanera'][0]['NumeroPedimento']=$pedimento['numero_pedimento'];
...
if($facturaParams['retencion_IVA']=="true"){
    ...
    $concepto['Impuestos']['Retenciones'][$index_retencion]['TasaOCuota'] = '0.106700';
    $importe_retencion = round(0.106700*$c['importe'],2);
    $concepto['Impuestos']['Retenciones'][$index_retencion]['Importe'] = $importe_retencion;
    $ivaRetenido += $importe_retencion;
    $index_retencion++;
}
...
$descuento =  $facturaParams['descuento'];
...
$hayRetencion = false;
    ...
    if($facturaParams['retener_ISR']=="true"){
        ...
        $retenido['importe'] = $isrRetenido;
        $datos['impuestos']['retenciones'][] = $retenido;

        $total_retenido+= $isrRetenido;

    }
    if($hayRetencion){
    $total =$datos['factura']['total'] ; // total incluyendo impuestos
    $total =(float)($total - $total_retenido);
    $datos['factura']['total']  = round($total,2);
    $datos['impuestos']['TotalImpuestosRetenidos']=round($total_retenido,2);
...
if($res['codigo_mf_numero']==0){
   ...
   $serie_id ="";
 if($ambientePruebas){
    $query="SELECT `idSerie` FROM `factura_series_prueba` WHERE `serie` = ?";
    $serie_id = $sql->get_bind_results($query,array("s",$facturaParams['serie']));
    $serie_id = $serie_id[0]['idSerie'];

   $query1="INSERT INTO `factura_generada_pruebas`
    (idSerie,version,`folio_factura`, `folio_impreso`, `idcliente`, `enviado`,
     `xml_file`, `montoTotal`, `uuid`, `fecha_timbrado`,`tipo`, `json_conceptos`) VALUES ('$serie_id','3.3','$folio','$folio','".$request['idCliente']."',
      '0','$xml_file','$total','".$res['uuid']."',NOW(),'$tipo','$tmpJConcpetos')";

      $query2="UPDATE `factura_series_prueba` SET `folio_actual` = `folio_actual` + 1 WHERE `serie` = ?";
...
$query1="INSERT INTO `factura_generada`
(idSerie,version,`folio_factura`, `folio_impreso`, `idcliente`, `enviado`,
 `xml_file`, `montoTotal`, `uuid`, `fecha_timbrado`,`tipo`, `json_conceptos`) VALUES ('$serie_id','3.3','$folio','$folio','".$request['idCliente']."',
  '0','$xml_file','$total','".$res['uuid']."',NOW(),'$tipo','$tmpJConcpetos')";

  $query2="UPDATE `factura_series` SET `folio_actual` = `folio_actual` + 1 WHERE `serie` = ?";

 }


$sql->ejecutarNoQuery($query1);

$sql->execQueryBinders($query2,array("s",$facturaParams['serie']));

//$respuesta['querys']=$query1.$query2;

  $errors = $sql->getErrorLog();
  $respuesta_texto =$res['codigo_mf_texto'];
  if(count($errors)>0){
    $respuesta_texto.="<br> Se timbro con exito, sin embargo Han ocurrido errores en la base de datos";

  }
  $respuesta['msg']=$respuesta_texto;

include("../wkWorks/generatePdf.php");
$tmp = explode(".",$xml_file);
$filePdf = $tmp[0].".pdf";
$path_pdf =($ambientePruebas)?'facturas/pruebas/pdf/':'../../'.$clientAndSystemFolder.'Facturacion/facturas/timbradas/pdf/';
$absolutePdf = $path_pdf.$filePdf;
$qr = $tmp[0].".png";
$EmitterPhone = $emisor['telefono'];
//generatePdf33($request['idCliente'],"var/www/luxline.com.mx/joseO/LuxFacturacion/Facturacion/facturas/timbradas/xml/".$xml_file,"goldman.badger@gmail.com",$EmitterPhone,"https://www.luxline.com.mx/joseO/LuxFacturacion/Facturacion/facturas/timbradas/xml/".$qr,$absolutePdf);
$folderXml="";
$pathQr="";
if($ambientePruebas){
$folderXml="pruebas/xml";
$pathXml = "facturas/".$folderXml."/".$xml_file;
$pathQr="https://www.luxline.com.mx/LuxFacturacion/Facturacion/prueba_qr_no_borrar.png";
}else{
    $folderXml="timbradas/xml";
    unset($_SESSION['conceptos']);
    $pathXml = "../../".$clientAndSystemFolder."Facturacion/facturas/".$folderXml."/".$xml_file;
    $pathQr ="https://www.luxline.com.mx/".$clientAndSystemFolder."Facturacion/facturas/".$folderXml."/".$qr;

}
$numCuentaPago ="N/A";
if(isset($facturaParams['numCuenta'])){
    $numCuentaPago  = $facturaParams['numCuenta'];
}
...
$numCuentaPago,
$absolutePdf,
$clientAndSystemFolder);

if(file_exists($absolutePdf)){
$respuesta['pdf']=$absolutePdf;
$respuesta['xml']=$pathXml;
}else{
 $respuesta_texto.="<br> No se pudo guardar PDF";
}
...
//unset($_SESSION['conceptos']);
    ...
    $respuesta['timbrado']=false;
    $respuesta['msg']="ha ocurrido un error fatal, contacta a soporte";
