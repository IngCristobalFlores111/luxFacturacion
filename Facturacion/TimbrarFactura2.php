...
$datos['emisor']['rfc'] = "AAA010101AAA"; //RFC DE PRUEBA CAMBIAR POR => $Factura["EmisorDatosRFC"]
...
$datos['factura']['subtotal'] = $Factura["Subtotal"]; // sin impuestos
$datos['factura']['subtotal'] = round($datos['factura']['subtotal'],3);


$datos['factura']['descuento'] = $Factura["Descuento"]; // descuento sin impuestos
...
"facturas/timbradas/pdf/".$_POST["ID"]."_".date("Y_m_d")."_".$_POST["FOLIO"].".pdf",$Factura["EmisorDatosEmail"],$Factura["EmisorDatosTelefono"],$Factura["ReceptorDatosEmail"],$Factura["ReceptorDatosTelefono"]
);
...
'INSERT INTO factura_generada(tipo,folio_impreso,folio_factura, idcliente, enviado, xml_file, montoTotal, uuid, fecha_timbrado, fecha_cancelada)
VALUES("'.$tipo.'","'.$folio_impreso.'","'.$_POST["FOLIO"].'","'.$_POST["ID"].'","0","'.$_POST["ID"].'_'.date("Y_m_d").'_'.$_POST["FOLIO"].'.xml"'.',"'.$datos["factura"]["total"].'","'.$res["uuid"].'","'.date("Y-m-d").'","");',MysqlLink::NAMED,MysqlLink::RET_NO);
?>