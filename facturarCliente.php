...
if($tipo_factura=='1'){
    $concepto['predial'] = $gj_facturaParams["predial"];
        ...
        if(isset($c['aduana'])){
            $concepto['numero'] = $c['numPedimento'];
            $concepto['fecha'] = $c['fechaPedimento'];
            $concepto['aduana'] = $c['aduana'];
            array_push($pedimentos,$c['idPedimento']);
        }
...
if($res['cancelada']=='NO')  // fue correctamente timbrada
{
    ...
    $jRespuesta =array("status"=>1,"respuesta"=>$res_corta,"folio"=>$folio);
 ...
 generatePdf("https://www.luxline.com.mx/joseO/LuxFacturacion/Facturacion/facturas/timbradas/xml/".$xml_file,"golman.badger@gmail.com",$EmitterPhone,"https://www.luxline.com.mx/joseO/LuxFacturacion/Facturacion/facturas/timbradas/xml/".$qr,$absolutePdf);
    $jconcepts = json_encode($conceptos,JSON_UNESCAPED_UNICODE|JSON_HEX_APOS|JSON_HEX_QUOT);
    $query ="INSERT INTO `factura_generada`(`tipo`,`folio_impreso`,`folio_factura`, `idcliente`, `enviado`, `xml_file`, `fecha_timbrado`,`montoTotal`, `uuid`,json_conceptos) VALUES ('$tipo_factura','$folio_impreso','$folio','$gs_idCliente','1','$xml_file','$date','$total','$uuid','$jconcepts')";
    ...
    if(count($pedimentos)!=0){
        foreach($pedimentos as $p){
            //$folio
            $query .="INSERT INTO `factura_pedimentos`(`idPedimento`, `folio_factura`) VALUES ('$p','$folio');";
...
}else
{
    ...
    $jRespuesta =array("status"=>0,"respuesta"=>$res_corta,"folio"=>$folio);

}
print_r(json_encode($jRespuesta,JSON_UNESCAPED_UNICODE|JSON_HEX_APOS|JSON_HEX_QUOT));
